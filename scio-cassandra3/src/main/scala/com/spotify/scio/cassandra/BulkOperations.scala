...
import com.datastax.driver.core.{Cluster, DataType, ProtocolVersion}
...
private[cassandra] class BulkOperations(val opts: CassandraOptions) extends Serializable {
  ...
  case class BulkConfig(protocol: ProtocolVersion, partitioner: String, numOfNodes: Int,
                        ...
                        dataTypes: Seq[Externalizer[DataType]])
  ...
  private val config = {
    ...
    val partitionKeyIndices = variables
      ...
      .map(_._2)
      .toArray
    val dataTypes = variables.map(v => Externalizer(v.getType))
    cluster.close()
  ...
  val serializeFn: Seq[AnyRef] => Array[ByteBuffer] = (values: Seq[AnyRef]) => {
